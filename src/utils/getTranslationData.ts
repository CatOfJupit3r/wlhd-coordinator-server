    ...
    try {
        ...
        translationsFiles.forEach((file) => {
            ...
            const manifest = fs.readFileSync(
                `${dlcFolderPath}/${file}/manifest.json`,
                'utf8'
            )

            const dlcDescriptor = (JSON.parse(manifest) as DLCManifest)
                .descriptor
            ...
            dlcContent.forEach((translationDir: string) => {
                ...
                const languageData = fs.readdirSync(
                    `${dlcFolderPath}/${file}/${translationDir}`
                )
